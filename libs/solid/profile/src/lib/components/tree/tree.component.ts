...
import { LazyTreeNode, Profile } from '../../state/profile.model';
import {
  GetChildren,
  GetEntries,
...
export class TreeComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  @Select(ProfileState.selectChildrenById) childrenById$!: Observable<{
    [id: number]: LazyTreeNode[];
  }>;
  @Select(ProfileState.selectEntries) entries$!: Observable<{
    [id: number]: Profile[];
  }>;
  ...
  public ngOnInit(): void {
    ...
    const childrenById = this._store.selectSnapshot(
      ProfileState.selectChildrenById,
    );
    const entriesById = this._store.selectSnapshot(ProfileState.selectEntries);
    const expandedNodeIds = this._store.selectSnapshot(
      ProfileState.selectExpandedNodeIds,
    );
    const selectedProfile = this._store.selectSnapshot(
      ProfileState.selectSelectedProfile,
    );
    const selectedProfilePath = this._store.selectSnapshot(
      ProfileState.selectSelectedProfilePath,
    );

    // console.log('childrenById', childrenById);
    // console.log('entriesById', entriesById);
    // console.log(
    //   'selected profile per url',
    ...
    // console.log(
    //   'oooooninit-openpath',
    //   this._store.selectSnapshot(ProfileState.selectSelectedProfilePath),
    // );
  ...
  public async ngAfterViewInit(): Promise<void> {
    ...
    this.TreeControl.expansionModel.changed.subscribe(async (change) => {
      ...
      if (
        this._store.selectSnapshot(ProfileState.selectNavigateProfileFromURL) &&
        this.openPath?.length
      )
        return;

      if (change.added) {
        // console.log(
        //   '(ngAfterViewInit) expanding bcs of change.added',
        //   change.added,
        // );

        const nodesToLoad = change.added.filter(
          (n) => n.type === 'category' && !n.loaded,
        );
        ...
        for (const node of nodesToLoad) {
          ...
          const newNode = this.TreeControl.dataNodes.find((n2) =>
            this.areNodesEqual(n2, node),
          );
  ...
  public async ngOnChanges(changes: SimpleChanges): Promise<void> {
    // console.log(
    //   'expanding selected node insicde ngOnChanges',
    //   this.selectedProfileId,
    // );
    // console.log('changes', changes);
    // console.log(
    //   'nnnavigateProfileFromURL',
    //   this._store.selectSnapshot(ProfileState.selectNavigateProfileFromURL),
    // );
    ...
    if (
      'openPath' in changes &&
      changes['selectedProfileId']?.currentValue !== -1 &&
      changes['openPath'].currentValue.length > 0
    ) {
      // console.log('change is openPath');
      // console.log('openPath', changes['openPath'].currentValue);
      const nodesToLoad = changes['openPath'].currentValue.filter(
        (n: LazyTreeNode) => n.type === 'category',
      );
    ...
    for (const node of nodesToLoad) {
      ...
      if (flatNode) {
        try {
          ...
          const newNode = this.TreeControl.dataNodes.find((n2) =>
            this.areNodesEqual(n2, flatNode),
          );
          ...
          // console.log(
          //   '(ngOnChanges) flat data (expansionModel)',
  ...
  private loadChildren(node: FlatTreeNode): Promise<void> {
    if (
      node.loaded ||
      !node.expandable ||
      (node?.type === 'category' && node.children && node.children.length > 0)
    ) {
  ...
  private updateNodeChildrenPatch(
    tree: LazyTreeNode[],
    nodeId: number,
    children?: LazyTreeNode[],
    entries?: Profile[],
  ...
  private updateNodeProfiles(
    ...
    return nodes.map((node) => {
      if (node.id === targetId) {
        ...
        return {
          ...node,
          children: this.updateNodeProfiles(
            node.children,
            targetId,
            newChildren,
            newProfiles,
